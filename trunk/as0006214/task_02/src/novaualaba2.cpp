...
public:
    ...
    double calcOutput(double currentOutput, double previousOutput, double input) const override {
        return coefA * currentOutput - coefB * pow(previousOutput, 2) + coefC * input + coefD * sin(input);
    }
};
...
int main() {
    ...
    for (int t = 0; t < 10; t++) {
        cout << "Iteration " << t + 1 << ":\n";
        cout << "Linear model output: " << currentOutput_linear_values[t] << '\n';
        cout << "Nonlinear model output: " << currentOutput_nonlinear_values[t] << '\n';
